...
ggplot() +
  geom_raster(data = severity_df, aes(x = x, y = y, 
    fill = burn_severity)) +
  scale_fill_gradientn(name = "Severity", colors = fire_colors(7), na.value = "white") +
...
#plot boundary
ggplot() + 
  geom_sf(data = boundary_sf, size = 1, color = "black") +
  ggtitle("Hayman Fire Scar") + 
  coord_sf() +
  theme_void()
...
plot(severity, rev(brewer.pal(n = 7, name = "YlOrRd")))

```

```{r}
# to plot raster in ggplot, need to convert to df first
ggplot() +
