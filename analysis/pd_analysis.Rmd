...
```{r plot_severity, fig.path = "../manuscript/figures", fig.cap = "Figure 1. Flammulated Owl nests sampled in the Hayman Fire burn scar (blue) plotted over Normalized Burn Ratios (NBR) depicting burn severity."}
...
```{r plot_sex, fig.path = "../manuscript/figures", fig.cap = "Figure 2. Mean prey deliveries during the incubation stage (top) and nestling stage (bottom). Males are shown in bue and females in red. Stars indicate non-significant differences in medians based on unpaired two-sample Wilcoxon tests."}
...
any(wilcox_inc_site$values <= 0.05) #test if any p.values are significantly different: FALSE
all(wilcox_inc_site$values > 0.05) #test if all p.values are not significant (double-checking first logical expression's validity): TRUE

# Nestling Stage
wilcox_nest_site <- testWilcox(pdMGSA_nest, pdHFSA_nest)
print(wilcox_nest_site)

any(wilcox_nest_site$values <= 0.05) #test if any p.values are significantly different: TRUE
dplyr::filter(wilcox_nest_site, values <= 0.05) #print which row has p.value that is significantly different
...
```{r plot_site, fig.path = "../manuscript/figures", fig.cap = "Figure 3. Mean prey deliveries during the incubation stage (top) and nestling stage (bottom). Missouri Gulch Study Area (MGSA) is shown in bue and Hayman Fire Study Area (HFSA) in red. Stars indicate significant differences in medians based on unpaired two-sample Wilcoxon tests."}
